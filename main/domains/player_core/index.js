...
function __hasMpvDll(dirPath){
  try {
    if (!dirPath) return false;
    return (
      fs.existsSync(path.join(dirPath, 'libmpv-2.dll')) ||
      fs.existsSync(path.join(dirPath, 'mpv-2.dll')) ||
      fs.existsSync(path.join(dirPath, 'mpv-1.dll'))
    );
  } catch {
    return false;
  }
...
function __resolveMpvDllDir(appRoot, isPackaged){
  if (process.platform !== 'win32') return '';
  try {
    const candidates = [];
    if (isPackaged) {
      candidates.push(path.join(process.resourcesPath, 'mpv', 'windows'));
    } else {
      candidates.push(path.join(appRoot, 'resources', 'mpv', 'windows'));
      let cur = appRoot;
      for (let i = 0; i < 5; i += 1) {
        const parent = path.dirname(cur);
        if (!parent || parent === cur) break;
        candidates.push(path.join(parent, 'resources', 'mpv', 'windows'));
        candidates.push(path.join(parent, 'app', 'resources', 'mpv', 'windows'));
        candidates.push(path.join(parent, 'app', 'dist', 'win-unpacked', 'resources', 'mpv', 'windows'));
        cur = parent;
      }
    }
    ...
    for (const candidate of candidates) {
      ...
      seen.add(key);
      if (__hasMpvDll(candidate)) return candidate;
    }
  } catch {}
  return '';
}
...
try {
  ...
  const bundledPlayerExe = (process.platform === 'win32')
    ...
    : '';
  // Dev + packaged parity: use bundled EXE whenever present (Python remains fallback).
  // This avoids brittle local Python dependency issues in dev runs.
  const canUseBundledExe = !!(bundledPlayerExe && fs.existsSync(bundledPlayerExe));

  // Ensure libmpv DLL discovery is deterministic (Python player reads this env var).
  const mpvDllDir = __resolveMpvDllDir(appRoot, isPackaged);
  ...
  __appendLog(`  mpvDllDir=${mpvDllDir || '(none)'}\n`);
...
function __restoreWindowAfterPlayerExit(ctx, exitCode, signal) {
  try {
    ...
    try {
      const { EVENT } = require('../../../shared/ipc');
      win.webContents.send(EVENT.BUILD14_PLAYER_EXITED, { exitCode, signal, synced });
    } catch (e) {
