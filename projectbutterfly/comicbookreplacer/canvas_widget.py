...
class CanvasWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self._fade_old_frame: QPixmap | None = None
    ...
    def begin_crossfade(self):
        """Capture current frame for crossfade transition."""
        if not self._fade_enabled or self.width() < 1 or self.height() < 1:
            return
        self._fade_old_frame = self.grab()
        self._fade_opacity = 1.0
        self._fade_timer.start()

    def _fade_tick(self):
        self._fade_opacity -= 0.12  # ~8 frames = ~130ms
        if self._fade_opacity <= 0.0:
            self._fade_opacity = 0.0
            self._fade_old_frame = None
    ...
    def paintEvent(self, event):
        ...
        # Crossfade old frame on top with fading opacity
        if self._fade_old_frame is not None and self._fade_opacity > 0.01:
            painter.setOpacity(self._fade_opacity)
            painter.drawPixmap(0, 0, self._fade_old_frame)
    ...
    def _paint_gutter_shadow(self, painter: QPainter, left_w: int, gutter: int):
        if gutter <= 0:
            return
        strength = float(self._state.settings.get("gutter_shadow_strength", 0.35))
