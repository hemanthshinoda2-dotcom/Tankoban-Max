...
from speed_slider_overlay import SpeedSliderOverlay
...
class ComicReaderWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.speed_slider_overlay = SpeedSliderOverlay(self)
        self.keys_overlay = KeysOverlay(self)
        ...
        self.speed_slider_overlay.hide()
        self.loupe.hide()
        ...
        self.speed_slider_overlay.speed_changed.connect(self._on_speed_slider_changed)
        ...
        self.bottom_hud.seek_preview.connect(self._on_hud_seek_preview)
        ...
        self.bottom_hud.navigate_clicked.connect(self._open_volume_nav)
    ...
    def _on_hud_seek_preview(self, idx: int):
        if not self.state.pages:
            return
        total = len(self.state.pages)
        self.bottom_hud.set_page_text(f"Page {int(idx) + 1}/{total}")
    ...
    def _toggle_image_fit(self):
        """Quick action: toggle image fit between height and width."""
        mode = self.get_control_mode()
        ...
        new_val = "width" if cur == "height" else "height"
        self.state.settings[key] = new_val
        self._toast(f"Fit {'Width' if new_val == 'width' else 'Height'}")
        self.canvas.update()
        self._emit_progress_changed()

    _WIDTH_PRESETS = [0.50, 0.60, 0.70, 0.74, 0.78, 0.90, 1.0]

    def _cycle_portrait_width(self):
        """Quick action: cycle portrait width through presets."""
        cur = float(self.state.settings.get("portrait_width_pct", 1.0))
        # Find next preset
        for p in self._WIDTH_PRESETS:
            if p > cur + 0.005:
                nxt = p
                break
        else:
            nxt = self._WIDTH_PRESETS[0]
        self.state.settings["portrait_width_pct"] = nxt
        self._toast(f"Width {int(nxt * 100)}%")
        self.canvas.update()
        self._emit_progress_changed()

    def _on_hud_gap_changed(self, delta: int):
        """Quick action: adjust two-page scroll row gap."""
        cur = int(self.state.settings.get("two_page_scroll_row_gap_px", 16))
        new_val = max(0, min(64, cur + delta))
        self.state.settings["two_page_scroll_row_gap_px"] = new_val
        self.bottom_hud.set_gap_value(new_val)
    ...
    def _update_hud_state(self):
        ...
        self.top_bar.set_bookmarked(self._is_bookmarked(int(self.state.page_index)))
        ...
        # Gap control visible only in twoPageScroll mode
        is_scroll_mode = self.get_control_mode() == "twoPageScroll"
        self.bottom_hud.set_gap_visible(is_scroll_mode)
        if is_scroll_mode:
            self.bottom_hud.set_gap_value(int(self.state.settings.get("two_page_scroll_row_gap_px", 16)))
    ...
    def _save_progress_now(self):
        ...
        max_seen_final = int(max(int(prev.get("max_page_seen", 0)), self.state.max_page_seen))
        payload = {
            ...
            "max_page_seen": max_seen_final,
            "bookmarks": sorted(int(i) for i in self._bookmarks),
            "finished": page_count > 0 and max_seen_final >= page_count - 1,
        }
        ...
        # Mirror progress to the library's shared progress.json
        self._sync_library_progress(payload)
    ...
    # --- Library Progress Sync ---

    def _library_book_id(self, path: str) -> str:
        """Generate book ID compatible with the library scanner's book_id_for_path."""
        try:
            ap = os.path.abspath(path)
            st = os.stat(ap)
            raw = "{}::{}::{}".format(ap, int(st.st_size), int(st.st_mtime * 1000))
            return base64.urlsafe_b64encode(raw.encode("utf-8")).decode("ascii").rstrip("=")
        except Exception:
            return ""

    def _library_progress_path(self) -> str | None:
        """Get the library's shared progress.json path."""
        try:
            # Try to import the shared storage module
            sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "shared"))
            import storage
            if storage._user_data_dir:
                return storage.data_path("progress.json")
        except Exception:
            pass
        # Fallback: try standard Tankoban QT data dir
        if os.name == "nt":
            base = os.environ.get("APPDATA", "")
            if base:
                return os.path.join(base, "Tankoban QT", "progress.json")
        return None

    def _sync_library_progress(self, reader_payload: dict):
        """Write progress to the library's shared progress.json in the format data_provider expects."""
        lib_path = self._library_progress_path()
        if not lib_path:
            return
        lib_id = self._library_book_id(self.state.book_path)
        if not lib_id:
            return

        page_count = int(reader_payload.get("page_count", 0))
        max_seen = int(reader_payload.get("max_page_seen", 0))
        finished = page_count > 0 and max_seen >= page_count - 1

        lib_entry = {
            "pageIndex": int(reader_payload.get("page_index", 0)),
            "pageCount": page_count,
            "maxPageIndexSeen": max_seen,
            "updatedAt": reader_payload.get("updated_at", time.time()),
            "finished": finished,
            "bookPath": self.state.book_path,
        }

        try:
            lib_db = read_json(lib_path, fallback={}) or {}
            lib_db[lib_id] = lib_entry
            write_json_debounced(lib_path, lib_db, delay_ms=300)
        except Exception:
            pass
    ...
    def _open_keys_overlay(self):
        if hasattr(self, "keys_overlay") and self.keys_overlay.isVisible():
            self.keys_overlay.close()
            return
        if hasattr(self, "keys_overlay"):
            self.keys_overlay.setGeometry(self.rect())
            self.keys_overlay.open()

    def _open_speed_slider(self):
        if self.speed_slider_overlay.is_open():
            self.speed_slider_overlay.close()
            return
        level = int(self.state.settings.get("auto_scroll_speed_level", 5))
        self.speed_slider_overlay.setGeometry(self.rect())
        self.speed_slider_overlay.open(level)

    def _on_speed_slider_changed(self, level: int):
        self.state.settings["auto_scroll_speed_level"] = level
        self._toast(f"Speed {level}/10", 600)
        self._emit_progress_changed()
    ...
    def resizeEvent(self, event):
        ...
        self._update_hud_geometry()
        self._update_hud_state()
        for overlay in (self.end_overlay, self.mega_settings_overlay, self.volume_nav_overlay, self.goto_page_overlay, self.speed_slider_overlay, self.keys_overlay):
    ...
    def close_book(self):
        ...
        self._update_hud_state()
        self._sync_window_title("Tankoban QT")
        try:
    ...
    def _set_title_for_page(self):
        if not self.state.pages:
            title = "Qt Comic Reader"
            self.setWindowTitle(title)
            self._sync_window_title(title)
            return
        total = len(self.state.pages)
        ...
        if self._is_flip_mode():
            ...
            if pair is None:
                ...
                title = f"{name} - Page {current}/{total}"
                self.setWindowTitle(title)
                self._sync_window_title(title)
                return
            if pair.left_index_or_none is not None:
                ...
                title = f"{name} - Pages {right}-{left}/{total}"
            else:
    ...
    def _sync_window_title(self, title: str):
        win = self.window()
    ...
    def go_to_page(self, index: int, keep_scroll: bool = False):
        ...
        # Trigger crossfade when page actually changes in flip modes
        if idx != self.state.page_index and self._is_flip_mode():
            self.canvas.begin_crossfade()
        mode = self.get_control_mode()
    ...
    def _create_loading_overlay(self):
        w = QWidget(self)
        w.setStyleSheet("background: rgba(0,0,0,180);")
        lbl = QLabel("Loading...", w)
        lbl.setStyleSheet(
    ...
    def toggle_manual_auto(self):
        """M key: toggle between Manual and Auto only (not full cycle)."""
        cur = self.get_control_mode()
        target = "auto" if cur == "manual" else "manual"
        self.state_machine.set_mode(target)
        self._toast(self._mode_label(target))
    ...
    def instant_replay(self):
        """Z key: restart current volume from page 0."""
        self.state.y = 0.0
        self.state.x = 0.0
        self.go_to_page(0, keep_scroll=False)
        self._toast("Replay")
    ...
    def reset_to_defaults(self):
        """Ctrl+0: reset all reader settings to defaults."""
        from state import default_reader_settings
        defaults = default_reader_settings()
        self.state.settings.update(defaults)
        self.canvas.update()
        self._toast("Settings reset")
        self._emit_progress_changed()
    ...
    def _handle_click_nav(self, x_pos):
        ...
        # Detect boundary → red blocked flash
        if self.state.pages:
            at_start = self.state.page_index <= 0
            at_end = self.state.page_index >= len(self.state.pages) - 1
            blocked = (go_next and at_end) or (not go_next and at_start)
        else:
            blocked = True

        self._flash_click_zone("right" if go_next else "left", blocked=blocked)
        if not blocked:
            if self._is_flip_mode():
                if go_next:
                    self.next_two_page()
    ...
    def handle_pointer_press(self, event) -> bool:
        ...
        if self._is_flip_mode() and self.is_mangaplus_mode() and self._mangaplus_zoom_pct() > 100:
            ...
            return True
        # Defer navigation so double-click (fullscreen) can cancel it
        self._pending_click_x = pt.x()
    ...
    def handle_pointer_release(self, event) -> bool:
        if event.button() != Qt.MouseButton.LeftButton:
            return False  # let right-clicks through for context menu
        if not self._mp_dragging:
            return bool(self.state.pages)
        self._mp_dragging = False
    ...
    def _on_single_click_confirmed(self):
        """Fires after double-click interval — confirmed single click = navigate."""
        x = getattr(self, "_pending_click_x", None)
        if x is not None and self.state.pages:
            self._handle_click_nav(x)

    def mouseMoveEvent(self, event):
        # Only show HUD when mouse is near the bottom edge of the widget
        pos = event.position()
        bottom_threshold = 80  # pixels from bottom edge
        if pos.y() >= self.height() - bottom_threshold:
            self.hud.note_activity()
        self._update_loupe(pos.toPoint())
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            self._single_click_timer.stop()  # cancel pending click navigation
    ...
    def contextMenuEvent(self, event):
        if self.state.pages:
            # Open mega settings as floater near cursor (Electron parity)
            local_pos = event.pos()
            self.mega_settings.setGeometry(self.rect())
            self.mega_settings.open_at(local_pos)
            event.accept()
            return
