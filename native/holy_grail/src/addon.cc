/*
 * mpv Holy Grail — Native Addon (Production)
 *
 ...
 *
 * Production additions over PoC:
 * - Property observation via mpv_observe_property + pollEvents()
 * - Track list querying via mpv_node conversion (getPropertyNode)
 * - Reinit support: destroyPlayer() keeps DLLs, destroyAll() frees everything
 */
...
enum {
    ...
    MPV_EVENT_SEEK              = 24,
    MPV_EVENT_PLAYBACK_RESTART  = 25,
    MPV_EVENT_PROPERTY_CHANGE   = 26,
};
...
// mpv_node for structured property access (track-list etc.)
...
typedef struct mpv_node_list {
    int         num;
    mpv_node*   values;
    char**      keys;    // null for arrays, non-null for maps
} mpv_node_list;
...
typedef int         (*pfn_mpv_observe_property)(mpv_handle*, uint64_t, const char*, int);
typedef mpv_event*  (*pfn_mpv_wait_event)(mpv_handle*, double);
typedef void        (*pfn_mpv_free_node_contents)(mpv_node*);
...
static bool    g_dllsLoaded = false;
...
static pfn_mpv_observe_property                p_mpv_observe_property = nullptr;
static pfn_mpv_wait_event                      p_mpv_wait_event = nullptr;
static pfn_mpv_free_node_contents              p_mpv_free_node_contents = nullptr;
...
static uint64_t g_nextObserveId = 1;
...
// Convert mpv_node tree to Napi::Value recursively
static Napi::Value MpvNodeToNapi(Napi::Env env, const mpv_node& node) {
    switch (node.format) {
        ...
        case MPV_FORMAT_INT64:
            return Napi::Number::New(env, (double)node.u.int64_);
        case MPV_FORMAT_DOUBLE:
            return Napi::Number::New(env, node.u.double_);
        case MPV_FORMAT_NODE_ARRAY: {
            if (!node.u.list) return env.Null();
            auto arr = Napi::Array::New(env, (size_t)node.u.list->num);
            for (int i = 0; i < node.u.list->num; i++) {
                arr.Set((uint32_t)i, MpvNodeToNapi(env, node.u.list->values[i]));
            }
            return arr;
        }
        case MPV_FORMAT_NODE_MAP: {
            ...
            for (int i = 0; i < node.u.list->num; i++) {
                ...
                if (key) {
                    obj.Set(key, MpvNodeToNapi(env, node.u.list->values[i]));
                }
            }
            return obj;
        }
...
static const char* LoadEgl(const char* path) {
    if (h_egl) return nullptr; // already loaded
...
static const char* LoadGles(const char* path) {
    if (h_gles) return nullptr; // already loaded
...
static const char* LoadMpv(const char* path) {
    if (h_mpv) return nullptr; // already loaded
    ...
    LOAD_FN(h_mpv, mpv_observe_property);
    LOAD_FN(h_mpv, mpv_wait_event);
    LOAD_FN(h_mpv, mpv_free_node_contents);
    return nullptr;
}
#undef LOAD_FN
...
Napi::Value InitGpu(const Napi::CallbackInfo& info) {
    ...
    // Load DLLs (skips if already loaded)
    const char* err;
...
// ═══════════════════════════════════════════════════════════════════
// § 12. N-API: command / getProperty / setProperty / getState
// ═══════════════════════════════════════════════════════════════════
...
Napi::Value GetState(const Napi::CallbackInfo& info) {
    ...
    double volume = 0;
    if (p_mpv_get_property(g_mpv, "volume", MPV_FORMAT_DOUBLE, &volume) >= 0)
        obj.Set("volume", volume);
    else
        obj.Set("volume", 100.0);

    int muted = 0;
    p_mpv_get_property(g_mpv, "mute", MPV_FORMAT_FLAG, &muted);
    obj.Set("muted", (bool)muted);

    double speed = 1.0;
    if (p_mpv_get_property(g_mpv, "speed", MPV_FORMAT_DOUBLE, &speed) >= 0)
        obj.Set("speed", speed);
    else
        obj.Set("speed", 1.0);
...
// ═══════════════════════════════════════════════════════════════════
// § 13. N-API: observeProperty(name) — subscribe to property changes
// ═══════════════════════════════════════════════════════════════════
...
Napi::Value ObserveProperty(const Napi::CallbackInfo& info) {
    ...
    // Choose observation format based on known property types
    int format = MPV_FORMAT_NONE; // default: let mpv pick, event carries no data (just notification)

    // Numeric properties → observe as double
    if (name == "time-pos" || name == "duration" || name == "volume" ||
        name == "speed" || name == "audio-delay" || name == "sub-delay" ||
        name == "video-aspect-override")
        format = MPV_FORMAT_DOUBLE;
    // Flag properties → observe as flag
    else if (name == "pause" || name == "eof-reached" || name == "mute" ||
             name == "sub-visibility")
        format = MPV_FORMAT_FLAG;
    // Complex properties → observe as node (track-list, chapter-list, video-params)
    else if (name == "track-list" || name == "chapter-list" || name == "video-params")
        format = MPV_FORMAT_NODE;

    uint64_t id = g_nextObserveId++;
    int err = p_mpv_observe_property(g_mpv, id, name.c_str(), format);
    if (err < 0) {
        Napi::Error::New(env, std::string("observe_property failed: ") + p_mpv_error_string(err))
            .ThrowAsJavaScriptException();
        return env.Undefined();
    }
...
// ═══════════════════════════════════════════════════════════════════
// § 14. N-API: pollEvents() → Array of events
// ═══════════════════════════════════════════════════════════════════
// Drains the mpv event queue (non-blocking). Returns an array of
// event objects: { event, name?, value?, ... }

Napi::Value PollEvents(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (!g_mpv) return Napi::Array::New(env, 0);

    auto events = Napi::Array::New(env);
    uint32_t idx = 0;
    const int MAX_EVENTS_PER_POLL = 64;

    for (int i = 0; i < MAX_EVENTS_PER_POLL; i++) {
        mpv_event* ev = p_mpv_wait_event(g_mpv, 0.0); // non-blocking
        ...
        switch (ev->event_id) {
            case MPV_EVENT_PROPERTY_CHANGE: {
                ...
                switch (prop->format) {
                    case MPV_FORMAT_DOUBLE:
                        if (prop->data)
                            obj.Set("value", Napi::Number::New(env, *(double*)prop->data));
                        break;
                    case MPV_FORMAT_FLAG:
                        if (prop->data)
                            obj.Set("value", Napi::Boolean::New(env, *(int*)prop->data != 0));
                        break;
                    case MPV_FORMAT_STRING:
                        if (prop->data && *(char**)prop->data)
                            obj.Set("value", Napi::String::New(env, *(char**)prop->data));
                        break;
                    case MPV_FORMAT_NODE:
                        if (prop->data)
                            obj.Set("value", MpvNodeToNapi(env, *(mpv_node*)prop->data));
                        break;
                    default:
                        // MPV_FORMAT_NONE — just a notification, no data
                        break;
                }
            ...
            case MPV_EVENT_FILE_LOADED:
                obj.Set("event", "file-loaded");
                events.Set(idx++, obj);
                break;

            case MPV_EVENT_END_FILE:
                obj.Set("event", "end-file");
                events.Set(idx++, obj);
                break;
            ...
            case MPV_EVENT_PLAYBACK_RESTART:
                obj.Set("event", "playback-restart");
                events.Set(idx++, obj);
                break;

            case MPV_EVENT_SHUTDOWN:
                obj.Set("event", "shutdown");
                events.Set(idx++, obj);
                break;
...
// ═══════════════════════════════════════════════════════════════════
// § 15. N-API: getPropertyNode(name) → structured JS value
// ═══════════════════════════════════════════════════════════════════
// Gets a property as mpv_node and converts to JS. Used for track-list,
// chapter-list, video-params, and other structured properties.

Napi::Value GetPropertyNode(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (!g_mpv) return env.Null();
    ...
    mpv_node node;
    int err = p_mpv_get_property(g_mpv, name.c_str(), MPV_FORMAT_NODE, &node);
    if (err < 0) return env.Null();

    Napi::Value result = MpvNodeToNapi(env, node);
    p_mpv_free_node_contents(&node);
    return result;
}

// ═══════════════════════════════════════════════════════════════════
// § 16. N-API: getHandle() / getSize()
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
// destroyPlayer: tears down mpv + ANGLE, keeps DLLs loaded for reinit
// destroyAll: also frees DLLs (for app quit)
...
// Legacy destroy() — same as destroyAll for backward compat
...
Napi::Object InitModule(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set("getPropertyNode",  Napi::Function::New(env, GetPropertyNode));
