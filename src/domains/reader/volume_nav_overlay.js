...
// -----------------------------
  ...
  // FIX-R09: books mode also uses the shared top-bar global search.
  function isBooksTopBarMode() {
    try { if (typeof appState !== 'undefined' && appState && appState.mode === 'books') return true; } catch {}
    try { return document.body.classList.contains('inBooksMode') || document.body.classList.contains('inBooksReader'); } catch {}
    return false;
  }
  ...
  function callGlobalSearchRender() {
    ...
    if (isBooksTopBarMode()) {
      try { window.booksApp?.renderGlobalSearchResults?.(); } catch {}
      return;
    }
  ...
  function callGlobalSearchHide() {
    ...
    if (isBooksTopBarMode()) {
      try { window.booksApp?.hideGlobalSearchResults?.(); } catch {}
      return;
    }
  ...
  function callGlobalSearchSetSelection(idx) {
    ...
    if (isBooksTopBarMode()) {
      try { window.booksApp?.setGlobalSearchSelection?.(idx); } catch {}
      return;
    }
  ...
  function callGlobalSearchActivateSelection() {
    ...
    if (isBooksTopBarMode()) {
      try { window.booksApp?.activateGlobalSearchSelection?.(); } catch {}
      return;
    }
