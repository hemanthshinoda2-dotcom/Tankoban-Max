...
class LoupeWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        center_x = w / 2.0
        center_y = h / 2.0
        glow = QRadialGradient(center_x, center_y, radius + glow_spread)
        glow.setColorAt(0.0, QColor(0, 0, 0, 0))
        glow.setColorAt(max(0.01, (radius - glow_spread) / (radius + glow_spread)), QColor(0, 0, 0, 0))
        glow.setColorAt(max(0.02, radius / (radius + glow_spread)), QColor(199, 167, 107, 80))
        glow.setColorAt(1.0, QColor(199, 167, 107, 0))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(glow)
        painter.drawEllipse(QRectF(-glow_spread, -glow_spread, w + 2 * glow_spread, h + 2 * glow_spread))
