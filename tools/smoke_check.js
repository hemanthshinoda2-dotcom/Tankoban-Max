...
function main() {
  ...
  checkFile('src/domains/books/reader/engine_foliate.js', { parse: true });
  checkFile('src/vendor/foliate/view.js');
  checkFile('src/vendor/foliate/LICENSE');
  ...
  // ===== X-F01: Books reader + TTS integration checks =====
  const booksReaderFiles = [
    // BUILD_OVERHAUL: modular reader replaces controller.js
    'src/domains/books/reader/reader_bus.js',
  ...
  for (const f of booksReaderFiles) {
    checkFile(f, { parse: true });
  }

  // Check reader UI element IDs exist in index.html
  ...
  for (const id of requiredIds) {
    if (!htmlContent.includes(`id="${id}"`)) {
      fail(`Missing reader UI element id="${id}" in src/index.html`);
      missingIds++;
    }
  }
  if (!missingIds) ok(`X-F01: All ${requiredIds.length} reader UI element IDs present`);

  // Check TTS engine registration pattern
  const ttsEdge = readText(path.join(ROOT, 'src', 'domains', 'books', 'reader', 'tts_engine_edge.js'));
  ...
  if (!ttsCore.includes('stepSegment')) {
    fail('TTS core missing stepSegment (TTS-F06)');
  }
